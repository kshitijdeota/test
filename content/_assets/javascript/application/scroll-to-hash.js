...
export default (hash) => {
  ...
  if (target) {
    ...
    //this needs to be calculated dynamically to see where the target is and then offset it
     const slideMoving = [
       { transform: `translateY(-${target.getBoundingClientRect().top}px)` },
    ...
    // if(target.className === 'exhibition-section--slide'){
    //   target.animate(slideMoving, slideTiming)
    // } else {
    //  scrollWindow(verticalOffset)
    //}
